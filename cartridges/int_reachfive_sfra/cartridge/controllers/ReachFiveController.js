...
var { isReachFiveSessionForcedAuth, getReachFiveUserCustomObjectType, getReachFiveProviderId } = require('*/cartridge/models/reachfiveSettings');
...
server.get('CallbackReachFiveRequest', function (req, res, next) {
    if (!empty(session.customer) && session.customer.authenticated && session.customer.registered) {
        // no need to log customer in if already logged in
    ...
    var codeVerifier = req.httpParameterMap.codeVerifier.value;

    if (codeVerifier === null || codeVerifier === undefined) {
        res.render('/r5-callback', {
            reachfiveSettings: reachfiveSettings,
        });
        return next();
    }
    ...
    if (error || error === '') {
        var message = !!req.httpParameterMap.error_description.value
            ? req.httpParameterMap.error_description.value
            : '';
        LOGGER.warn('access denied: reach5 response: ' + message);

        loginFailed(message, res);
        return next();
    }

    //  Step 3: Exchange authorization code for ID token
    var authorizationResponse = reachFiveService.exchangeAuthorizationCodeForIDToken({ code, codeVerifier });   

    if (!authorizationResponse.ok) {
        LOGGER.warn('authorization error : for code ' + code);
        res.json({
            success: false,
            msg: authorizationResponse.errorMessage
        });
        // loginFailed('genericerror', res);
        return next();
    }

    var reachfiveSession = new ReachfiveSessionModel(authorizationResponse.object);
    ...
    LOGGER.debug(
        'Parsed UserId "{0}" from response: {1}',
        externalID,
        reachfiveSession.profile
    );

    // Check if an account with reachfive provider id exist
    var profile = CustomerMgr.getExternallyAuthenticatedCustomerProfile(
        reachFiveProviderId,
        externalID
    );
    ...
    if (empty(profile)) {
        ...
        if (
            ...
            var socialNameResponse = !empty(reachfiveSession.profile.auth_type)
                ? reachfiveSession.profile.auth_type
                : '';

            var profilesLinked = false;
            if (reachfiveSession.has_password) {
                profilesLinked = ReachFiveModel.setExternalParams(
                    externalID,
            ...
            if (profilesLinked) {
                loggedCustomer = ReachFiveModel.loginReachFiveCustomer(
                    externalID,
                    existingCustomer.profile
                );
                target = handleCustomerRoute(
                    loggedCustomer,
                    target,
                    stateObj.handleCustomerRoute
                );
            } else {
                target = URLUtils.https(
            ...
            if (!empty(email)) {
                profile = CustomerMgr.searchProfiles(
                    'email = {0}',
                    'lastLoginTime desc',
                    email
                ).first();
            ...
            if (profile !== null) {
                ...
                if (
                    ...
                    target = URLUtils.url(
                        'ReachFiveController-StartPrefillRegister',
                        'rurl',
                        rurl
                    ).toString();
...
server.get(
    ...
    function (req, res, next) {
        ...
        res.render('account/login/reachfivelinkform', {
            ...
            userName: userName,
            actionUrl: URLUtils.url(
                'ReachFiveController-HandleLinkForm',
                'rurl',
                rurl
            ).toString(),
    ...
    function (req, res, next) {
        ...
        if (authenticatedCustomer && authenticatedCustomer.authenticated) {
            ...
            if (externalID) {
                ...
                ReachFiveModel.setExternalParams(
                    externalID,
                    customer.getProfile()
                );
            }

            var target = afterAuth.getLoginRedirectURL(
            ...
            target = handleCustomerRoute(
                ...
                target,
                afterAuth.isHandlerActionRequire(req.querystring.rurl)
            );
 ...
 * @param {string} error - errorCode or errorMsg Error code
 * @param {Object} res Response Object
 * */
function loginFailed(error, res) {
    var errorMsg = Resource.msg(
        'reachfive.' + error,
        'reachfive',
        'Error during process login'
    );
    if (errorMsg === error) {
        // Ressource not found so we take the error message
        errorMsg = error;
    }
    res.json({
        success: false,
        msg: errorMsg,
        redirectUrl: URLUtils.url('Login-Show', 'errormsg', errorMsg)
    });
    ...
    function (req, res, next) {
        ...
        if (
            ...
            res.render('/account/loginPrefill', {
                ...
                actionUrl: URLUtils.url(
                    'ReachFiveController-SubmitProfileSocialLogin',
                    'rurl',
                    rurl
                ).toString()
            });
        ...
        if (session.privacy.prefill_register) {
            ...
            if (profileForm.valid) {
                ...
                this.on('route:BeforeComplete', function (req, res) {
                    ...
                    var externalID = reachfiveSession.profile.sub.trim();
                    var profile = ReachFiveModel.createReachFiveCustomer(
                        externalID,
                        {
                    ...
                    if (!empty(profile)) {
                        ...
                        if (loggedCustomer) {
                            ...
                            equalList.profile = sessionModel.equal(
                                customerModel,
                                profileFields
                            );
                            if (formInfo.phone) {
                            ...
                            equalList.trigger = !(
                                equalList.profile && equalList.phone
                            );

                            if (equalList.trigger) {
                                var tknStatus = reachFiveService.verifySessionAccessTkn(
                                        true
                                    );
                            ...
                            target = handleCustomerRoute(
                                loggedCustomer,
                                target,
                                afterAuth.isHandlerActionRequire(
                                    req.querystring.rurl
                                )
                            );
...
server.post('UserUpdate', function (req, res, next) {
    try {
        ...
        var reachFiveUserCustomObjectType = getReachFiveUserCustomObjectType();
        ...
        var userCustomObj = CustomObjectMgr.getCustomObject(
            reachFiveUserCustomObjectType,
            user.id
        );
        if (empty(userCustomObj)) {
            Transaction.wrap(function () {
                userCustomObj = CustomObjectMgr.createCustomObject(
                    reachFiveUserCustomObjectType,
                    user.id
                );
            });
...
server.get('UncachedContext', function (req, res, next) {
    var context = {};
    ...
    if (
        ...
        isSessionAuthRequired = isReachFiveSessionForcedAuth
