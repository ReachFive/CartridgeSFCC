...
server.replace(
    ...
    function (req, res, next) {
        ...
        if (
            ...
            || !finalizeOAuthLoginResult.userInfoResponse
        ) {
        ...
        if (!response && oauthProviderID.indexOf('reachfive') !== -1) {
            ...
            httpClient.setRequestHeader(
                'Authorization',
                'Bearer '
                    + finalizeOAuthLoginResult.accessTokenResponse.accessToken
            );
            httpClient.open(
                'GET',
                'https://sandbox-tif.reach5.net/identity/v1/userinfo'
            );
