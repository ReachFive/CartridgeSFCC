/* eslint-disable */
// @TODO need to be refactored and lint
'use strict';
...
var ReachFiveModel = {
    ...
    reachFiveProviderId: reachfiveSettings.getReachFiveProviderId(),
    ...
    createReachFiveCustomer: function (
        ...
        try {
            ...
            if (
                reachFiveHelper.isFieldExist(externalProfile, 'email')
                && reachfiveSettings.isReachFiveEmailAsLogin
            ) {
                ...
                var PipeletPasswordGeneration = new dw.system.Pipelet(
                ...
                credentials.setAuthenticationProviderID(
                    this.reachFiveProviderId
                );
                credentials.setExternalID(externalID);

                try {
                    // we add email for this new external customer profile
                    var externalCustomerProfile = newCustomer.getExternalProfile(
                            this.reachFiveProviderId,
                            externalID
                        );
                    ...
                    LOGGER.debug(
                        'Error on added email to external profile {0}',
                        e
                    );
                }

                LOGGER.info(
                    'Customer created with credentials and an external profile {0} with the external ID {1}',
                    this.reachFiveProviderId,
                    externalID
                );
            } else {
                ...
                LOGGER.info(
                    'Customer created with an external profile {0} with the external ID {1}',
                    this.reachFiveProviderId,
                    externalID
                );
            }
    ...
    loginReachFiveCustomer: function (externalID, profile) {
        ...
        if (!credentials.isEnabled()) {
            LOGGER.warn(
                'Customer attempting to login into a disabled profile: {0} with external id: {1}',
            ...
            Transaction.wrap(function () {
                currentcustomer = CustomerMgr.loginExternallyAuthenticatedCustomer(
                        reachFiveProviderId,
                        externalID,
                        true
                    );
    ...
    setExternalParams: function (externalID, profile) {
        ...
        try {
            ...
            credentials.setExternalID(externalID);
            LOGGER.debug(
                'Modified customer {0} external parameters',
                externalID
            );
            Transaction.commit();
            return true;
            ...
            LOGGER.error(
                "set external paramaters on customer's credentials with id: {0} failed with this exception : {1}",
                externalID,
                e
            );
            Transaction.rollback();
